...
export class SurveyWinConsoPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.idTable =+ this.navParams.get('idSurvey').toString().slice(3,6); // slice for select what i want in the new array
    this.iteratorWin =+ this.navParams.get('idSurvey').toString().slice(6);
